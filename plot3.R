...
##        and February 2, 2007.  We need the "Date", "Time", "Sub_metering_1",
##        "Sub_metering_2", and "Sub_metering_3" columns
...
##        Setup the plot area with no points
dev.off()
plot(x = formattedDateTime, y = Sub_metering_1, type = "n", xlab = "", ylab = "Energy sub metering")

##        Add the lines and legend for each power subset
##        Legend is inset slightly to avoid cutting off text
...
##        The legend ends up cutoff using dev.copy, so let's just create the
##        plot from scratch on the png display device
